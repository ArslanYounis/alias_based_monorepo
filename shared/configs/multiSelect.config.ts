...
const controls: {
  [K in keyof Omit<MultiSelectProps, "language">]?: ControlDefinition<MultiSelectProps>;
} & { fieldKey?: ControlDefinition<MultiSelectProps>; bindingData?: ControlDefinition<MultiSelectProps>; propsOverride?: ControlDefinition<MultiSelectProps> } = {
  fieldKey: { type: ["text"], label: "Field Key", hasArabic: false, isRequired: true, unique: true, defaultValue: "" },
  label: { type: ["text", "code"], label: "Label", hasArabic: true, defaultValue: "Multi Select", defaultValueAr: "Multi Select", defaultCode: "return 'Multi Select'", defaultCodeAr: "return 'Multi Select'" },
  ...
  maxSelection: { type: ["number", "code"], label: "Max Selection", defaultValue: undefined, defaultCode: "return undefined" },
  showAddButton: { type: ["boolean", "code"], label: "Show Add Button?", defaultValue: false, defaultCode: "return false" },
  onChange: { type: ["code"], label: "onChange", defaultCode: 'console.log("input changes", eventData)', isEvent: true, visibility: (props: MultiSelectProps) => !props.disabled },
