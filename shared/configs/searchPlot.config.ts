...
const controls: {
  [K in keyof Omit<SearchPlotProps, "language">]?: ControlDefinition<SearchPlotProps>;
} & {
  propsOverride?: ControlDefinition<SearchPlotProps>;
} = {
  title: {
    type: ["text", "code"],
    label: "Title",
  ...
  enabledTabs: {
    type: ["code"],
    label: "Enabled Tabs",
    defaultValue: { plot: true, company: true, owner: true },
    defaultCode: `return {
  plot: true,
  company: true,
  owner: true
