...
const controls: {
  [K in keyof Omit<RadioFieldProps, "language">]?: ControlDefinition<RadioFieldProps>;
} & { fieldKey?: ControlDefinition<RadioFieldProps>; propsOverride?: ControlDefinition<RadioFieldProps> } = {
  fieldKey: { type: ["text"], label: "Field Key", hasArabic: false, isRequired: true, unique: true, defaultValue: "" },
  label: { type: ["text", "code"], label: "Label", hasArabic: true, defaultValue: "RadioField", defaultValueAr: "RadioField", defaultCode: "return 'RadioField'", defaultCodeAr: "return 'RadioField'" },
  required: { type: ["boolean", "code"], label: "Required?", defaultValue: false, defaultCode: "return false" },
  onChange: { type: ["code"], label: "onChange", defaultCode: 'console.log("radio changed", eventData)', isEvent: true, visibility: (props: RadioFieldProps) => !props.disabled },
